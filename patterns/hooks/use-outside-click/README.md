...
특정 요소 바깥의 pointer 이벤트를 감지해 닫기 동작을 실행합니다.
...
팝오버와 메뉴에서 바깥 클릭 처리 로직이 중복되고 cleanup 누락이 생기기 쉽습니다.
...
`Use Outside Click`의 핵심은 특정 요소 바깥의 pointer 이벤트를 감지해 닫기 동작을 실행하는 방식입니다. 컴포넌트마다 반복되는 이벤트, 타이머, 비동기, DOM 구독 로직을 재사용 경계로 묶을 때 유용합니다.

## 기본 코드 형태

```tsx
export function useUseOutsideClick(input: Input) {
...
이 패턴은 "반복되는 절차 로직을 어떤 훅 API로 감출 것인가"에 대한 답입니다. JSX 재사용보다 상태 전이, 구독, cleanup, 이벤트 정책 재사용이 핵심이면 `Use Outside Click` 패턴을 봅니다.

패턴 유형으로는 `DOM Event Hook`에 가깝습니다. 같은 카테고리의 다른 패턴과 비교할 때 "API 모양", "상태 소유권", "변경 영향 범위"를 기준으로 구분합니다.
