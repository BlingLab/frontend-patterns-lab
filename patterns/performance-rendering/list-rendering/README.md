...
수천 개 항목을 한 번에 렌더하면 초기 표시와 스크롤 성능이 나빠집니다.
...
`List Rendering`의 핵심은 큰 목록의 렌더 비용을 줄이고 key 안정성을 보장하는 방식입니다. Profiler나 사용자 체감으로 느린 지점이 확인된 뒤 렌더 범위와 계산 비용을 줄일 때 사용합니다.
...
패턴 유형으로는 `List Performance`에 가깝습니다. 같은 카테고리의 다른 패턴과 비교할 때 "API 모양", "상태 소유권", "변경 영향 범위"를 기준으로 구분합니다.
