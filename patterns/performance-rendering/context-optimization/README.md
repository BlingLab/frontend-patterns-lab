...
큰 Context 하나에 자주 바뀌는 값을 넣으면 모든 소비자가 함께 렌더됩니다.
...
`Context Optimization`의 핵심은 Context value와 Provider 범위를 쪼개 불필요한 렌더를 줄입니다. Profiler나 사용자 체감으로 느린 지점이 확인된 뒤 렌더 범위와 계산 비용을 줄일 때 사용합니다.
...
패턴 유형으로는 `Context Performance`에 가깝습니다. 같은 카테고리의 다른 패턴과 비교할 때 "API 모양", "상태 소유권", "변경 영향 범위"를 기준으로 구분합니다.
