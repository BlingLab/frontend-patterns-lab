...
"느리다"는 느낌만으로 useCallback을 추가하는 것은 측정 없는 최적화입니다. 실제로 어떤 컴포넌트가 몇 번 렌더되는지 측정한 뒤 병목이 확인된 곳만 최적화해야 합니다.
...
- useRef로 렌더 횟수를 세는 useRenderCount 훅을 만든다
- React DevTools Profiler로 렌더 원인을 분석한다
- 측정 없는 최적화는 복잡도만 올린다
...
function useRenderCount(label: string) {
  ...
  useEffect(() => {
    console.debug(label, 'rendered', renderCount.current);
  });
}
```
