...
측정 없이 최적화하면 실제 병목이 아닌 곳에 복잡도만 추가됩니다.

이 문서는 패턴 이름을 외우기 위한 것이 아니라, 리뷰 중 "이 책임은 어디에 있어야 하는가"를 판단하기 위한 기준입니다.

## 언제 사용하는가

- 어떤 컴포넌트가 자주 렌더되는지 확인할 때
- memo 적용 전후 효과를 비교할 때
- Context 변경 영향 범위를 확인할 때

## 언제 피해야 하는가

- 프로덕션 코드에 디버그 로그를 남기는 경우
- 렌더 횟수만 보고 UX 성능을 단정하는 경우

## 어떻게 사용하는가

1. React DevTools Profiler를 우선 사용한다
2. 필요하면 useRenderCount 같은 보조 훅을 쓴다
3. 측정 결과를 기준으로 최적화한다

## 실무 예시

`Render Tracking`의 핵심은 렌더 횟수와 원인을 관찰해 최적화 대상을 찾습니다. Profiler나 사용자 체감으로 느린 지점이 확인된 뒤 렌더 범위와 계산 비용을 줄일 때 사용합니다.
...
패턴 유형으로는 `Observability`에 가깝습니다. 같은 카테고리의 다른 패턴과 비교할 때 "API 모양", "상태 소유권", "변경 영향 범위"를 기준으로 구분합니다.
...
- [Memoization Boundary](../memoization-boundary/README.md)
- [Context Optimization](../context-optimization/README.md)
