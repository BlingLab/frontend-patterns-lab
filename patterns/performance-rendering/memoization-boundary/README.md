...
작은 컴포넌트마다 memo를 붙이면 코드만 복잡해지고 실제 이득은 작을 수 있습니다.
...
`Memoization Boundary`의 핵심은 memoization을 비용이 큰 하위 트리의 경계에 둡니다. Profiler나 사용자 체감으로 느린 지점이 확인된 뒤 렌더 범위와 계산 비용을 줄일 때 사용합니다.
...
패턴 유형으로는 `Memo Boundary`에 가깝습니다. 같은 카테고리의 다른 패턴과 비교할 때 "API 모양", "상태 소유권", "변경 영향 범위"를 기준으로 구분합니다.
