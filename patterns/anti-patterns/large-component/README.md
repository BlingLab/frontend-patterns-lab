...
`Large Component`의 핵심은 하나의 컴포넌트가 데이터, 상태, 표현, side effect를 모두 갖지 않게 하는 방식입니다. 코드 리뷰에서 반복되는 구조적 냄새를 설명하고 작은 리팩터링 단위로 쪼갤 때 사용합니다.
...
형태상으로는 `Responsibility Smell`에 속하므로, 같은 카테고리의 다른 패턴과 비교할 때 "API 모양", "상태 소유권", "변경 영향 범위"를 기준으로 구분합니다.
