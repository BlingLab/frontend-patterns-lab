...
계산 가능한 값을 effect와 state로 중복 저장하지 않습니다.
...
원본 값이 바뀔 때마다 effect로 파생 state를 맞추면 한 렌더 늦고 동기화 버그가 생깁니다.
...
`Use Effect For Derived State`의 핵심은 계산 가능한 값을 effect와 state로 중복 저장하는 문제를 피하는 것입니다. 코드 리뷰에서 반복되는 구조적 냄새를 설명하고 작은 리팩터링 단위로 쪼갤 때 사용합니다.
...
이 문서는 금지 규칙이 아니라 리팩터링 신호입니다. `Use Effect For Derived State`가 보이면 왜 생겼는지 확인하고, 더 작은 책임 경계로 옮길 수 있는지 봅니다.

형태상으로는 `Derived State Smell`에 속하므로, 같은 카테고리의 다른 패턴과 비교할 때 "API 모양", "상태 소유권", "변경 영향 범위"를 기준으로 구분합니다.
