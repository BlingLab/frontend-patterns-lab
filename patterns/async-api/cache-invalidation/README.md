...
상품을 삭제했는데 목록 화면이 여전히 삭제된 상품을 보여주면 사용자가 혼란스럽습니다. mutation 후 관련 query cache를 무효화하면 다음 조회 시 최신 데이터를 가져옵니다.
...
- mutation onSuccess에서 관련 queryKey를 invalidate한다
- 무효화된 쿼리는 다음 마운트/포커스 시 자동으로 재요청된다
- 어떤 쿼리를 무효화할지 명시적으로 선언하면 사이드이펙트가 추적된다
