...
- 같은 동작 로직을 완전히 다른 시각 디자인에 재사용한다
- Radix UI, Headless UI 같은 라이브러리가 이 방식으로 설계됐다
- 복잡한 접근성 로직을 중앙화하고 마크업은 자유롭게 남긴다
...
- 마크업을 완전히 바꿔도 상태 전이와 접근성 props가 유지되는지 확인합니다.
- 키보드 조작, focus 관리, aria 속성을 실제 브라우저에서 확인합니다.
- 호출부가 필수 props getter를 누락했을 때 실패가 조용히 숨지 않는지 봅니다.
