...
- theme, auth, locale 같은 앱 전역 관심사에 적합하다
- 소비자 컴포넌트만 리렌더되고 중간 컴포넌트는 무관하다
- context value가 자주 바뀌면 소비 범위를 나눠 최적화해야 한다

## 언제 사용하는가

- 현재 사용자, 테마, locale처럼 하위 트리 여러 곳에서 같은 값을 읽을 때
- 중간 컴포넌트가 쓰지 않는 props를 계속 전달해 전달 경로가 불필요하게 길어질 때
...
- Provider 범위가 너무 넓어 불필요한 rerender가 퍼지지 않는지 확인합니다.
- Context value identity가 매 렌더 새로 만들어지지 않는지 확인합니다.
- Provider 없이 consumer를 호출했을 때 기본값이 버그를 숨기지 않는지 봅니다.
