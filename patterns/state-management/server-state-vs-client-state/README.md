...
원격 데이터는 server state로, 화면 제어 값은 client state로 분리합니다.

## 패턴 형태

- 분류: 상태 관리
- 형태: State Classification
- 목적: 상태를 어디에 두고 어떻게 흐르게 할 것인가

## 왜 필요한가

서버 데이터와 클라이언트 UI 상태를 같은 store에 섞으면 캐시, 동기화, 낙관 업데이트 기준이 흐려집니다.
...
`Server State vs Client State`의 핵심은 원격 데이터는 server state로, 화면 제어 값은 client state로 분리하는 방식입니다. 목록 필터, 선택 상태, 편집 플로우처럼 값의 소유자가 헷갈리는 화면에서 먼저 검토합니다.
...
이 패턴은 "이 값의 소유자는 누구인가"에 대한 답입니다. 값이 서버, URL, 부모, 컴포넌트 내부 중 어디에 속하는지 먼저 나눈 뒤 `Server State vs Client State` 패턴을 선택합니다.

패턴 유형으로는 `State Classification`에 가깝습니다. 같은 카테고리의 다른 패턴과 비교할 때 "API 모양", "상태 소유권", "변경 영향 범위"를 기준으로 구분합니다.
